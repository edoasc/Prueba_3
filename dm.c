...
void crear_red_sc(sys *obj, double densidad){
  ...
  int nc = ceil(pow((double)obj->n, 1.0/3.0));
...
void crear_velocidades_gaussiana(sys *obj, double temp, int seed){
  ...
  for (int i = 0; i < (obj->n); ++i) {
    obj->v[i][0] = gsl_ran_gaussian(rg, sqrt(temp)); // desviacion estandar = srqt(2)
    obj->v[i][1] = gsl_ran_gaussian(rg, sqrt(temp));
    obj->v[i][2] = gsl_ran_gaussian(rg, sqrt(temp));
  } 
  gsl_rng_free(rg);
}
...
void termodinamica(sys *obj){
  ...
  // Presion instantanea
  obj->press = ((obj->n)/(obj->vol)*(obj->temp) + (obj->aux->virial)/(obj->vol));
}
  ...
  for(int t = 1; t < npasos + 1; ++t){
    ...
    if(obj->aux->termostat == 1){
      if( (t >= obj->aux->termostat_ini) &&  (t <= obj->aux->termostat_fin) ){
        if ((t-obj->aux->termostat_ini) % (obj->aux->termostat_freq) == 0) {
...
void graficar_distribuciones(sys *obj) {

    FILE *gnuplot_x = popen("gnuplot", "w");

    fprintf(gnuplot_x, "set terminal png\n");
    fprintf(gnuplot_x, "set output 'distribucion_x.png'\n");
    fprintf(gnuplot_x, "set title 'Distribución de velocidades en el eje x'\n");
    fprintf(gnuplot_x, "set xlabel 'Velocidad en x'\n");
    fprintf(gnuplot_x, "set ylabel 'Frecuencia'\n");
    fprintf(gnuplot_x, "set grid\n");
    fprintf(gnuplot_x, "binwidth = 0.05\n");
    fprintf(gnuplot_x, "bin(x, width) = width * floor(x / width)\n");
    fprintf(gnuplot_x, "plot '-' using (bin($1,binwidth)):(1.0) smooth freq with boxes title 'Velocidad en x'\n");
    for (int i = 0; i < obj->n; ++i) {
        fprintf(gnuplot_x, "%f\n", obj->v[i][0]);
    }
    fprintf(gnuplot_x, "e\n");
    ...
    FILE *gnuplot_y = popen("gnuplot", "w");

    fprintf(gnuplot_y, "set terminal png\n");
    fprintf(gnuplot_y, "set output 'distribucion_y.png'\n");
    fprintf(gnuplot_y, "set title 'Distribución de velocidades en el eje y'\n");
    fprintf(gnuplot_y, "set xlabel 'Velocidad en y'\n");
    fprintf(gnuplot_y, "set ylabel 'Frecuencia'\n");
    fprintf(gnuplot_y, "set grid\n");
    fprintf(gnuplot_y, "binwidth = 0.05\n");
    fprintf(gnuplot_y, "bin(x, width) = width * floor(x / width)\n");
    fprintf(gnuplot_y, "plot '-' using (bin($1,binwidth)):(1.0) smooth freq with boxes title 'Velocidad en y'\n");
    for (int i = 0; i < obj->n; ++i) {
        fprintf(gnuplot_y, "%f\n", obj->v[i][1]);
    }
    fprintf(gnuplot_y, "e\n");
    ...
    FILE *gnuplot_z = popen("gnuplot", "w");

    fprintf(gnuplot_z, "set terminal png\n");
    fprintf(gnuplot_z, "set output 'distribucion_z.png'\n");
    fprintf(gnuplot_z, "set title 'Distribución de velocidades en el eje z'\n");
    fprintf(gnuplot_z, "set xlabel 'Velocidad en z'\n");
    fprintf(gnuplot_z, "set ylabel 'Frecuencia'\n");
    fprintf(gnuplot_z, "set grid\n");
    fprintf(gnuplot_z, "binwidth = 0.05\n");
    fprintf(gnuplot_z, "bin(x, width) = width * floor(x / width)\n");
    fprintf(gnuplot_z, "plot '-' using (bin($1,binwidth)):(1.0) smooth freq with boxes title 'Velocidad en z'\n");
    for (int i = 0; i < obj->n; ++i) {
        fprintf(gnuplot_z, "%f\n", obj->v[i][2]);
    }
    fprintf(gnuplot_z, "e\n");
    ...
    FILE *gnuplot_mag = popen("gnuplot", "w");

    fprintf(gnuplot_mag, "set terminal png\n");
    fprintf(gnuplot_mag, "set output 'distribucion_magnitudes.png'\n");
    fprintf(gnuplot_mag, "set title 'Magnitud de velocidades '\n");
    fprintf(gnuplot_mag, "set xlabel 'Velocidad'\n");
    fprintf(gnuplot_mag, "set ylabel 'Frecuencia'\n");
    fprintf(gnuplot_mag, "set grid\n");
    fprintf(gnuplot_mag, "binwidth = 0.05\n");
    fprintf(gnuplot_mag, "bin(x, width) = width * floor(x / width)\n");
    fprintf(gnuplot_mag, "plot '-' using (bin($1,binwidth)):(1.0) smooth freq with boxes title 'Velocidad'\n");

    double magnitude = 0;
    for (int i = 0; i < obj->n; ++i) {
      magnitude = sqrt(pow(obj->v[i][0], 2) + pow(obj->v[i][1], 2) + pow(obj->v[i][2], 2));
      fprintf(gnuplot_mag, "%f\n", magnitude);
    }
    fprintf(gnuplot_mag, "e\n");
    pclose(gnuplot_mag);

    remove("velocidades.dat");
}